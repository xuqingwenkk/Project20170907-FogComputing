...
PLUGINLIB_DECLARE_CLASS(nodelet_front_feature, FrontFeature, nodelet_front_feature::FrontFeature, nodelet::Nodelet);

namespace nodelet_front_feature
{
    void FrontFeature::onInit() {
        ros::NodeHandle& private_nh = getPrivateNodeHandle();
        pub = private_nh.advertise<fog_msgs::Features>("features", 1);
        image_transport::ImageTransport it(private_nh);
        sub = it.subscribe("/camera/image", 1, &FrontFeature::imageCallback, this);
    ...
    void FrontFeature::imageCallback(const sensor_msgs::ImageConstPtr &image) {
    ...
    void FrontFeature::Load_Calib(string path2calibrations, cCamModelGeneral_ &cam_model) {
