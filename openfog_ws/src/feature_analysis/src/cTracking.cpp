	...
	const double& timestamp)
{
	...
	if (mState == WORKING || mState == LOST)
        ...
        cout << "Go To Loop and Mapping" << endl;
	if (!loopAndMapperSet)
	{
		mpLocalMapper->SetMatcherProperties(mCurrentFrame.DescDims(),
			mCurrentFrame.HavingMasks());  // Assign values to descDim and HavingMasks
...
cv::Matx44d cTracking::GrabImageSet(const fog_msgs::Features &features,
							 const double& timestamp)
{
	std::chrono::system_clock Time;

	if (mState == WORKING || mState == LOST)
		mCurrentFrame = cMultiFrame(features,
									timestamp, mp_mdBRIEF_extractorOct, mpORBVocabulary,
									camSystem, imgCounter - 1);
	else
		mCurrentFrame = cMultiFrame(features,
...
cv::Matx44d cTracking::GrabImageSet(const cv::Mat &descriptors,
                                    const cv::Mat &masks,
                                    ...
                                    const double &timestamp)
{
    ...
    if (mState == WORKING || mState == LOST)
        mCurrentFrame = cMultiFrame(descriptors, masks, keypoints,
                                    timestamp, mp_mdBRIEF_extractorOct, mpORBVocabulary,
                                    camSystem, imgCounter - 1);
    else
        mCurrentFrame = cMultiFrame(descriptors, masks, keypoints,
...
bool cTracking::Track()
{
    ...
    {
        std::cout << "Enter First Initialization for NOT_INITIALIZED" << std::endl;
        FirstInitialization();
        std::cout << "Finish First Initialization " << std::endl;
    }
    else if (mState == INITIALIZING)
    {
        std::cout << "Enter Initialize for INITIALIZING" << std::endl;
        Initialize();
    }
    else
    {
        ...
        if (mState == WORKING && !RelocalisationRequested())
        {
			...
			{
                                std::cout << "Tracking Previous Frame 1" << std::endl;
            ...
            else
            {
				...
				if (!bOK){
					bOK = TrackPreviousFrame();
                                        std::cout << "Tracking Previous Frame 2" << std::endl;
                                }
            }
        }
        else
    ...
    std::cout << "update drawer" << std::endl;
	return true;
}
...
void cTracking::Initialize()
{
    ...
    std::cout << "There are " << nmatches << "correspondences" << std::endl;
    ...
    std::cout << "After Triangulated, the number of nmatches is" << nmatches << std::endl;
...
bool cTracking::TrackPreviousFrame()
{
   ...
   // Optimize pose again with all correspondences
	...
	cout << "There is " << nmatches << " left" << endl;
...
bool cTracking::TrackWithMotionModel()
{
	...
	int nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 50);
        ...
        cout << "Tracking with Motion Model: (nmatches before optimization) " << nmatches << endl;
    ...
    cout << "Tracking with Motion Model: (nmatches after optimization) " << nmatches << endl;
...
bool cTracking::TrackLocalMap()
{
	...
	mnMatchesInliers = cOptimizer::PoseOptimization(&mCurrentFrame, inliers);
        ...
        cout << "Tracking Local Map mnMatchesInliers: " << mnMatchesInliers <<endl;
