...
// 		}
	...
	cv::Matx44d cSystem::TrackMultiColSLAM(const fog_msgs::Features &features, const double &timestamp)
	{
		{
			unique_lock<mutex> lock(mMutexReset);
			if (mbReset)
		...
		return mpTracker->GrabImageSet(features, timestamp);
	}

	cv::Matx44d cSystem::TrackMultiColSLAM(const cv::Mat& descriptors,
								  const cv::Mat& masks,
								  ...
								  const double &timestamp)
	{
		...
		return mpTracker->GrabImageSet(descriptors, masks, keypoints, timestamp);
	}

	void cSystem::ActivateLocalizationMode()
	{
            //unlock when destruct unique_lock
		unique_lock<mutex> lock(mMutexMode);
