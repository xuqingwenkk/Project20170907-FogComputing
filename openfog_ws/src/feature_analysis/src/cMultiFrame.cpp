...
/*********Feature Detection***********/
		...
		for (int c = 0; c < nrCams; ++c)
		{
			...
			cout << "The number of key features is " << keyRaysTemp[c].size() << endl;
		}
	...
	cMultiFrame::cMultiFrame(const fog_msgs::Features& features_,
				const double &timeStamp,
				std::vector<mdBRIEFextractorOct*> extractor,
				ORBVocabulary* voc,
				cMultiCamSys_& camSystem_,
...
//			(*mp_mdBRIEF_extractorOct[c])(images[c], camModel.GetMirrorMask(0),
//										  keyPtsTemp[c], camModel, mDescriptors[c], mDescriptorMasks[c]);
			keyPtsTemp[c].reserve(features.keypoints.size());
			for(size_t i = 0; i < features.keypoints.size(); i++){
				keyPtsTemp[c].at(i).angle = features.keypoints.at(i).angle;
				keyPtsTemp[c].at(i).class_id = features.keypoints.at(i).class_id;
				keyPtsTemp[c].at(i).octave = features.keypoints.at(i).octave;
				keyPtsTemp[c].at(i).pt.x = features.keypoints.at(i).ptx;
				keyPtsTemp[c].at(i).pt.y = features.keypoints.at(i).pty;
				keyPtsTemp[c].at(i).response = features.keypoints.at(i).response;
				keyPtsTemp[c].at(i).size = features.keypoints.at(i).size;
			}
			mDescriptors[c] = cv_bridge::toCvCopy(features.descriptor.descriptors, "mono8")->image;
			mDescriptorMasks[c] = cv_bridge::toCvCopy(features.descriptor.masks, "mono8")->image;
...
/*********Feature Detection***********/
	...
	cMultiFrame::cMultiFrame(const cv::Mat& descriptors,
				const cv::Mat& masks,
			...
			mp_mdBRIEF_extractorOct(extractor),
			mpORBvocabulary(voc),
			fea_descriptors(descriptors),
			fea_masks(masks),
			fea_keypoints(keypoints),
			mTimeStamp(timeStamp),
			camSystem(camSystem_),
			mdBRIEF(true),
			imgCnt(_imgCnt)
...
//										  keyPtsTemp[c], camModel, mDescriptors[c], mDescriptorMasks[c]);
		keyPtsTemp[c].reserve(features.keypoints.size());
		keyPtsTemp[c] = fea_keypoints;
		mDescriptors[c] = fea_descriptors;
		mDescriptorMasks[c] = fea_masks;
        std::cout << "keyPtsTemp[c] size is ";
        std::cout << keyPtsTemp[c].size() << std::endl;
