...
namespace OmniSLAM
{
    ...
    BriefExtractor::BriefExtractor(int nfeatures,
                                   float scale_factor,
                                   int nlevels,
                                   int edge_threshold,
                                   int first_level,
                                   ...
                                   bool learn_masks,
                                   int desc_size) :
            nfeatures_(nfeatures), scale_factor_(scale_factor), num_levels_(nlevels),
            edge_threshold_(edge_threshold), first_level_(first_level),
            score_type_(score_type), patch_size_(patch_size), fast_threshold_(fast_threshold),
            use_agast_(use_agast), fast_agast_type_(fast_agast_type), learn_masks_(learn_masks),
            desc_size_(desc_size), do_dBrief_(do_dBrief)
    {
        mv_scale_factor_.resize(num_levels_);
        mv_scale_factor_[0] = 1;
        for (int i = 1; i < num_levels_; i++)
            mv_scale_factor_[i] = mv_scale_factor_[i - 1] * scale_factor_;

        double invScaleFactor = 1.0 / scale_factor_;
        mv_inv_scale_factor_.resize(num_levels_);
        mv_inv_scale_factor_[0] = 1;
        for (int i = 1; i < num_levels_; i++)
            mv_inv_scale_factor_[i] = mv_inv_scale_factor_[i - 1] * invScaleFactor;

        mv_image_pyramid_.resize(num_levels_);
        mv_mask_pyramid_.resize(num_levels_);

        mn_features_per_level_.resize(num_levels_);
        double factor = (1.0 / scale_factor_);
        double nDesiredFeaturesPerScale = nfeatures*(1 - factor) /
                                          (1 - pow(factor, num_levels_));

        int sumFeatures = 0;
        for (int level = 0; level < num_levels_ - 1; level++)
        {
            mn_features_per_level_[level] = cvRound(nDesiredFeaturesPerScale);
            sumFeatures += mn_features_per_level_[level];
            nDesiredFeaturesPerScale *= factor;
        }
        mn_features_per_level_[num_levels_ - 1] = std::max(nfeatures - sumFeatures, 0);

        const int npoints = 2 * 8 * desc_size_;
    ...
    int BriefExtractor::DefaultNorm() const {
        return NORM_HAMMING;
    }

    int BriefExtractor::DescriptorSize() const {
        return desc_size_;
    }

    int BriefExtractor::DescriptorType() const {
        return CV_8U;
    }
...
#undef GET_VALUE_MASK2
    ...
    vector<cv::KeyPoint> BriefExtractor::DistributeOctTree(
    ...
    void BriefExtractor::ComputeKeyPointsOctTree(
            vector<vector<KeyPoint> >& allKeypoints)
    {
        allKeypoints.resize(num_levels_);

        const double W = 30.0;
        Ptr<AgastFeatureDetector> ag =
                AgastFeatureDetector::create(fast_threshold_, true, fast_agast_type_);
        Ptr<FastFeatureDetector> fd =
                FastFeatureDetector::create(fast_threshold_, true, fast_agast_type_);

        for (int level = 0; level < num_levels_; ++level)
        {
            const int minBorderX = EDGE_THRESHOLD - 3;
            const int minBorderY = minBorderX;
            const int maxBorderX = mv_image_pyramid_[level].cols - EDGE_THRESHOLD + 3;
            const int maxBorderY = mv_image_pyramid_[level].rows - EDGE_THRESHOLD + 3;
            ...
            for (int i = 0; i<nRows; i++)
            {
                ...
                for (int j = 0; j<nCols; j++)
                {
                    ...
                    if (use_agast_)
                        ag->detect(mv_image_pyramid_[level].rowRange(iniY, maxY).colRange(iniX, maxX),
                                   vKeysCell, mv_mask_pyramid_[level].rowRange(iniY, maxY).colRange(iniX, maxX));
                    else
                        fd->detect(mv_image_pyramid_[level].rowRange(iniY, maxY).colRange(iniX, maxX),
                                   vKeysCell, mv_mask_pyramid_[level].rowRange(iniY, maxY).colRange(iniX, maxX));
            ...
            keypoints = DistributeOctTree(vToDistributeKeys,
                                          ...
                                          mn_features_per_level_[level], level);

            const int scaledPatchSize = PATCH_SIZE * mv_scale_factor_[level];
        ...
        for (int level = 0; level < num_levels_; ++level)
            computeOrientation(mv_image_pyramid_[level], allKeypoints[level], umax_);
    }

    void BriefExtractor::ComputeKeyPointsOld(
            std::vector<std::vector<KeyPoint> > &allKeypoints)
    {
        allKeypoints.resize(num_levels_);

        double imageRatio = (double)mv_image_pyramid_[0].cols / mv_image_pyramid_[0].rows;

        for (int level = 0; level < num_levels_; ++level)
        {
            const int nDesiredFeatures = mn_features_per_level_[level];
            ...
            const int minBorderY = minBorderX;
            const int maxBorderX = mv_image_pyramid_[level].cols - EDGE_THRESHOLD;
            const int maxBorderY = mv_image_pyramid_[level].rows - EDGE_THRESHOLD;
            ...
            for (int i = 0; i<levelRows; i++)
            {
                ...
                for (int j = 0; j<levelCols; j++)
                {
                    ...
                    Mat cellImage = mv_image_pyramid_[level].rowRange(iniY, iniY + hY).colRange(iniX, iniX + hX);
    ...
    void BriefExtractor::ComputePyramid(
            cv::Mat image,
            cv::Mat Mask)
    {
        for (int level = 0; level < num_levels_; ++level)
        {
            double scale = mv_inv_scale_factor_[level];
            Size sz(cvRound((double)image.cols*scale), cvRound((double)image.rows*scale));
            Size wholeSize(sz.width + EDGE_THRESHOLD * 2, sz.height + EDGE_THRESHOLD * 2);
            Mat temp(wholeSize, image.type()), masktemp;
            mv_image_pyramid_[level] = temp(Rect(EDGE_THRESHOLD, EDGE_THRESHOLD, sz.width, sz.height));

            if (!Mask.empty())
            {
                masktemp = Mat(wholeSize, Mask.type());
                mv_mask_pyramid_[level] = masktemp(Rect(EDGE_THRESHOLD, EDGE_THRESHOLD, sz.width, sz.height));
            }

            // Compute the resized image
            if (level != 0)
            {
                resize(mv_image_pyramid_[level - 1], mv_image_pyramid_[level], sz, 0, 0, INTER_LINEAR);
                if (!Mask.empty())
                {
                    resize(mv_mask_pyramid_[level - 1], mv_mask_pyramid_[level], sz, 0, 0, INTER_NEAREST);
                }

                copyMakeBorder(mv_image_pyramid_[level], temp, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD,
                               BORDER_REFLECT_101 + BORDER_ISOLATED);
                if (!Mask.empty())
                    copyMakeBorder(mv_mask_pyramid_[level], masktemp, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD,
...
//        else
//        {
//            _descriptors.create(nkeypoints, desc_size_, CV_8U);
//            _descriptorMasks.create(nkeypoints, desc_size_, CV_8U);
...
//            Mat& workingMat = mv_image_pyramid_[level];
...
//            float scale = mv_scale_factor_[level];
//            if (do_dBrief_)
//            {
...
//                               desc, descMasks, camModel, pattern_, this->learn_masks_,
//                               this->do_dBrief_,this->desc_size_);
//
